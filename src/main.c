...
void simulation_tick(const int tick_number) {
    ...
    int used_emergency_lanes[3] = {0, 0, 0};
    const int emergency_code = process_emergency_queue(used_emergency_lanes);

    if (emergency_code == -1) {
        LOG_ERROR("Invalid emergency code returned from process_emergency_queue: %d", emergency_code);
        return;
    }
    if (!array_contains_int(used_emergency_lanes, 3, 1))
        process_landing_queue(&landing_queue_1, 1);
    if (!array_contains_int(used_emergency_lanes, 3, 2))
        process_landing_queue(&landing_queue_2, 2);
    if (!array_contains_int(used_emergency_lanes, 3, 3))
